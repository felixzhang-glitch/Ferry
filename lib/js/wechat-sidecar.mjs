...
// Disabled until run() initializes it; importing this module never reads secrets.
let reportObservabilityDelivery = () => {};

function loadObservabilityConfig(readFile = fs.readFileSync) {
  try {
    const values = {};
    const raw = readFile(path.join(ROOT_DIR, "conf/.env.observability"), "utf8");
    for (const line of raw.split(/\r?\n/)) {
      // Literal values only. Never evaluate dotenv/shell syntax or retain other keys.
      const match = line.match(/^\s*(OBSERVABILITY_ENABLED|OBSERVABILITY_INGEST_TOKEN)\s*=\s*(?:"([A-Za-z0-9_-]*)"|'([A-Za-z0-9_-]*)'|([A-Za-z0-9_-]*))\s*(?:#.*)?$/);
      if (match) values[match[1]] = match[2] ?? match[3] ?? match[4];
    }
    return {
      enabled: /^(1|true|yes|on)$/i.test(values.OBSERVABILITY_ENABLED || ""),
      token: values.OBSERVABILITY_INGEST_TOKEN || "",
    };
  } catch {
...
function createObservabilityReporter(config, request = http.request) {
  let inflight = 0;
  return (event) => {
    let timer;
    let counted = false;
    ...
    try {
      if (!config.enabled || !/^[A-Za-z0-9_-]+$/.test(config.token || "") || inflight >= 8) return;
      if (!/^[0-9a-f]{32}$/.test(event.run_id || "")) return;
      if (!["success", "error", "partial"].includes(event.status)) return;
      if (!Number.isSafeInteger(event.count) || event.count < 0) return;
      if (!Number.isFinite(event.duration_seconds) || event.duration_seconds < 0) return;
      const body = JSON.stringify({
        kind: "delivery", channel: "wechat", source: "interactive",
        run_id: event.run_id, status: event.status,
        count: event.count, duration_seconds: event.duration_seconds,
      });
      inflight += 1;
      counted = true;
      const req = request("http://127.0.0.1:38080/internal/observability/events", {
        method: "POST",
        headers: {
        ...
        res.on("end", finish);
        res.resume(); // Discard every response body, including errors.
      });
      req.on("error", finish);
      timer = setTimeout(() => {
        req.destroy();
      ...
      finish(); // Best effort, no logging, retry or impact on message delivery.
    }
  };
}

async function deliverReplies(replies, sendReply, runId, report = reportObservabilityDelivery) {
  ...
  try {
    ...
    status = count ? "partial" : "error";
    throw error;
  } finally {
    // Deduplicated/file-only responses contain no outgoing batch to measure.
    if (count || status !== "success") {
      try {
        report({ run_id: runId, status, count, duration_seconds: (performance.now() - started) / 1000 });
      } catch { /* Observation must not change the original send result. */ }
    }
  }
}
...
async function sendMessageItem(account, toUserId, item, contextToken = "") {
  const clientId = `ferry-${Date.now()}-${crypto.randomBytes(4).toString("hex")}`;
  ...
  return new Promise((resolve, reject) => {
    ...
    if (timeoutMs > 0) {
      req.setTimeout(timeoutMs, () =>
        req.destroy(new Error(`ferry webhook timeout after ${timeoutMs}ms`)),
      );
    }
    req.on("error", reject);
    req.end(bodyText);
...
async function postToCodexClaw(account, msg, text, files = [], images = []) {
  ...
  console.log(`ferry replied message_id=${body.message_id} chunks=${replies.length}`);
  await deliverReplies(
    replies,
    (replyText) => sendText(account, msg.from_user_id, replyText, contextToken),
    data.observability_run_id,
  );
}
...
async function run() {
  reportObservabilityDelivery = createObservabilityReporter(loadObservabilityConfig());
