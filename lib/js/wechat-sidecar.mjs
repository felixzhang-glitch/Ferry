...
function collectFileItems(msg) {
  return (msg.item_list || []).filter((item) => item.type === 2 || item.type === 4 || item.type === 5);
}
...
async function postToCodexClaw(account, msg, text, files = []) {
...
async function handleInbound(account, msg) {
  ...
  try {
    ...
    console.log(
      `inbound from=${redact(msg.from_user_id)} id=${id} text=${JSON.stringify(text)} files=${archived.length}`,
    );
    await postToCodexClaw(account, msg, text, archived);
  } catch (error) {
    console.error(`failed to process inbound id=${id}: ${String(error)}`);
    if (!text && archived.length > 0) {
      // The file is safely on disk and already acked; only the session notice
      // was lost, so "服务繁忙" would misreport a successful receipt.
      console.warn(`files archived and acked but webhook failed id=${id}; session notice lost`);
    } else {
      try {
