...
`PI_IDLE_TIMEOUT_SECONDS` 限制等待下一行 stdout 的空闲时间；`PI_TIMEOUT_SECONDS` 限制每次 CLI 尝试的总耗时，持续输出不会刷新该预算，stdout EOF 后的进程退出与 stderr 等待也计入。超时、取消及关闭流会回收进程；服务关闭会终止仍注册的进程。
